...
set INPUT=%1
if [%INPUT%]==[] set INPUT=how_are_you_doing.wav
...
if NOT EXIST "%AUDIO_MODELS_PATH%\" (
    echo [91mERROR: Invalid model directory %AUDIO_MODELS_PATH% , execute download_audio_models.sh to download models[0m
    EXIT /B 1
)
...
set "MODEL_PATH=%AUDIO_MODELS_PATH%\audio_models\aclnet\FP32\%MODEL_NAME%.xml"
