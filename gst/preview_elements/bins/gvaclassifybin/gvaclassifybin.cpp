...
/* TODO: create gvainferencebin, inherit gvaclassifybin and gvadetectbin from it
and change needed properties, e.g. roi-list */

#define gva_classify_bin_parent_class parent_class
G_DEFINE_TYPE(GvaClassifyBin, gva_classify_bin, GST_TYPE_BIN);

static void gva_classify_bin_finalize(GObject *object);
static void gva_classify_bin_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);
static void gva_classify_bin_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec);

static GstStateChangeReturn gva_classify_bin_change_state(GstElement *element, GstStateChange transition);

static void gva_classify_bin_class_init(GvaClassifyBinClass *klass) {
    GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
    GstElementClass *element_class = GST_ELEMENT_CLASS(klass);

    element_class->change_state = gva_classify_bin_change_state;

    gobject_class->set_property = gva_classify_bin_set_property;
    gobject_class->get_property = gva_classify_bin_get_property;
    ...
    g_object_class_install_property(
        ...
        g_param_spec_enum("converter-type", "Converter Type", "Postprocessing converter type",
                          ...
                          static_cast<int>(post_processing::ConverterType::TO_TENSOR), prm_flags));
    g_object_class_install_property(
        gobject_class, PROP_ROI_LIST,
        g_param_spec_boolean("roi-list", "ROI List", "Inference on ROI list", true, prm_flags));
...
static void gva_classify_bin_init(GvaClassifyBin *self) {
    ...
    new (&self->props) GvaClassifyBin::_Props();
...
void gva_classify_bin_finish_init(GvaClassifyBin * /* self */) {
}
...
static void gva_classify_bin_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec) {
    ...
    case PROP_MODEL:
        ...
        GST_INFO_OBJECT(self, "model property set: %s", self->props.model.c_str());
        g_object_set(G_OBJECT(self->inference), "model", self->props.model.c_str(), nullptr);
        break;
    case PROP_IE_CONFIG:
        self->props.ie_config = g_value_get_string(value);
        GST_INFO_OBJECT(self, "ie-config property set: %s", self->props.ie_config.c_str());
    ...
    case PROP_DEVICE:
        ...
        GST_INFO_OBJECT(self, "device property set: %s", self->props.device.c_str());
    ...
    case PROP_NIREQ:
        ...
        GST_INFO_OBJECT(self, "nireq property set: %u", self->props.nireq);
        g_object_set(G_OBJECT(self->inference), "nireq", self->props.nireq, nullptr);
        break;
    case PROP_BATCH_SIZE:
        self->props.batch_size = g_value_get_uint(value);
        GST_INFO_OBJECT(self, "batch-size property set: %u", self->props.batch_size);
    ...
    case PROP_OBJECT_CLASS:
        ...
        GST_INFO_OBJECT(self, "tensorinference object-class property set: %s", self->props.object_class.c_str());
        g_object_set(G_OBJECT(self->inference), "object-class", self->props.object_class.c_str(), nullptr);
        break;
...
static GstStateChangeReturn gva_classify_bin_change_state(GstElement *element, GstStateChange transition) {
    GvaClassifyBin *self = GVA_CLASSIFY_BIN(element);
