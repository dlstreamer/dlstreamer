...
static void gva_action_recognition_bin_init(GvaActionRecognitionBin *self) {
    ...
    self->queue3 = gst_element_factory_make("queue", nullptr);
    ...
    self->aggregate = gst_element_factory_make("meta_aggregate", nullptr);

    gst_bin_add_many(GST_BIN(self), self->tee, self->queue1, self->queue2, self->queue3, self->preproc,
                     self->encoder_inference, self->acc, self->decoder_inference, self->postproc, self->aggregate,
                     nullptr);
...
static bool link_elements(GvaActionRecognitionBin *self) {
    // tee[0] -> queue2 -> preproc -> queue3 -> enc_infer -> acc -> dec_infer -> postproc
    if (!gst_element_link_many(self->tee, self->queue2, self->preproc, self->queue3, self->encoder_inference, self->acc,
