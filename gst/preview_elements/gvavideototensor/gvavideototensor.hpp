...
typedef struct _GvaVideoToTensor {
    ...
    GstFlowReturn split_rois(GstBuffer *buf) const;
    GstFlowReturn run_preproc(GstBuffer *buf) const;
    GstFlowReturn run_preproc(GstBuffer *inbuf, GstBuffer *outbuf) const;
} GvaVideoToTensor;
