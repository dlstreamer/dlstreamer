...
class HumanPoseEstimator {
  public:
    ...
    HumanPoseEstimator(const std::string &modelPath, const std::string &targetDeviceName);
    std::vector<HumanPose> estimate(const cv::Mat &image);
    ~HumanPoseEstimator();

  private:
    void preprocess(const cv::Mat &image, uint8_t *buffer) const;
    std::vector<HumanPose> postprocess(const float *heatMapsData, const int heatMapOffset, const int nHeatMaps,
    ...
    void correctCoordinates(std::vector<HumanPose> &poses, const cv::Size &featureMapsSize,
                            const cv::Size &imageSize) const;
