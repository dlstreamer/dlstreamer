...
const std::string ATSSConverter::boxes_layer_name = "boxes";
const std::string ATSSConverter::labels_layer_name = "labels";

void ATSSConverter::parseOutputBlob(const InferenceBackend::OutputBlob::Ptr &boxes_blob,
    ...
    if (object_size != ATSSConverter::model_object_size)
        throw std::invalid_argument("Object size dimension of output blob is set to " + std::to_string(object_size) +
                                    ", but only " + std::to_string(ATSSConverter::model_object_size) + " supported.");
...
TensorsTable ATSSConverter::convert(const OutputBlobs &output_blobs) const {
