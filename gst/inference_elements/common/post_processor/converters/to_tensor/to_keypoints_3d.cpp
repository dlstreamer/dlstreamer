...
TensorsTable ToKeypoints3DConverter::convert(const OutputBlobs &output_blobs) const {
    ...
    try {
        ...
        for (const auto &blob_iter : output_blobs) {
            ...
            const float *points_data = reinterpret_cast<const float *>(blob->GetData());
            if (not points_data)
                throw std::invalid_argument("Output blob data is nullptr");

            for (size_t i = 0; i < points_num; ++i) {
                auto current_point = points_data + i * dimension;
