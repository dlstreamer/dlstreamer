...
class RendererNV12 : public RendererYUV {
  public:
    RendererNV12(const std::vector<cv::Scalar> &rgb_colors, double Kb, double Kr) {
    ...
    void draw_rectangle(std::vector<std::shared_ptr<cv::Mat>> mats, Color c, cv::Point2i bbox_min,
        ...
        cv::rectangle(u_v, bbox_min / 2, bbox_max / 2, {yuv_color[1], yuv_color[2]}, 1);
    ...
    void draw_circle(std::vector<std::shared_ptr<cv::Mat>> mats, Color rgb_color, cv::Point2i pos,
        ...
        cv::circle(u_v, pos_u_v, thickness / 2, {yuv_color[1], yuv_color[2]}, -1);
    }
    ...
    void draw_text(std::vector<std::shared_ptr<cv::Mat>> mats, Color rgb_color, cv::Point2i pos,
        ...
        cv::Point2i pos_u_v(pos.x / 2, pos.y / 2);
        cv::putText(u_v, text, pos_u_v, cv::FONT_HERSHEY_TRIPLEX, 0.5, {yuv_color[1], yuv_color[2]}, 1);
    }
};
