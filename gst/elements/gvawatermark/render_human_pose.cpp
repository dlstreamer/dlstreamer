...
void renderHumanPose(const std::vector<float>& poses, cv::Mat& image) {
    ...
    size_t human_num = poses.size() / 18 / 2;
    for (size_t i = 0; i < poses.size(); i += human_num) {
        // CV_Assert(pose.keypoints.size() == 18);

        for (size_t keypointIdx = i; keypointIdx < i + 18; keypointIdx += 2) {
        //     if (pose.keypoints[keypointIdx] != absentKeypoint) {
        cv::circle(image, cv::Point(poses[keypointIdx], poses[keypointIdx + 1]), 4, colors[keypointIdx % 18], -1);
            // }
        }
    }
