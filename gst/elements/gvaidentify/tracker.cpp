...
TrackerParams::TrackerParams()
    : min_track_duration(25), forget_delay(150), affinity_thr(0.85), shape_affinity_w(0.5), motion_affinity_w(0.2),
      min_det_conf(0.0), averaging_window_size(1), bbox_aspect_ratios_range(0.666, 5.0), bbox_heights_range(1, 1280),
      drop_forgotten_tracks(true), max_num_objects_in_track(300) {
}
...
void Tracker::Process(cv::Size frame_size, const TrackedObjects &detections, int frame_idx) {
    if (frame_size_ == cv::Size()) {
        frame_size_ = frame_size;
    } else {
        CV_Assert(frame_size_ == frame_size);
    }
