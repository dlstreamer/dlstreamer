...
class IterativeSpeedometer : public Speedometer {
  ...
  public:
    IterativeSpeedometer(double interval, double alpha, double alpha_hw, double speedlimit, int speedlimit_violations,
                         bool print_each_stream = true)
        : interval(interval), alpha(alpha), alpha_hw(alpha_hw), speedlimit(speedlimit),
          speedlimit_violations(speedlimit_violations), print_each_stream(print_each_stream) {
    }
    ...
    try {
        ...
        for (unsigned counter = 0; counter < intervals_list.size(); counter++) {
            ...
            if (not speedometers.count(interval)) {
                std::shared_ptr<Speedometer> speedometer = std::shared_ptr<Speedometer>(
                    new IterativeSpeedometer(std::stod(interval), std::stod(alpha), std::stod(alpha_hw),
                                             std::stod(speedlim), std::stoi(speedlim_viols)));
