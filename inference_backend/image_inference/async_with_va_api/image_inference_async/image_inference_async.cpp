...
ImageInferenceAsync::ImageInferenceAsync(const InferenceBackend::InferenceConfig &config, VaApiDisplayPtr va_display,
...
void ImageInferenceAsync::SubmitInference(VaApiImage *va_api_image, IFrameBase::Ptr user_data,
    ...
    if (!user_data)
        throw std::invalid_argument("Invalid user_data object");
...
void ImageInferenceAsync::SubmitImage(const Image &src_image, IFrameBase::Ptr user_data,
    ...
    InferenceBackend::ImageTransformationParams::Ptr image_transform_params =
        user_data ? user_data->GetImageTransformationParams() : nullptr;
