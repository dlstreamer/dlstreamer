...
void IrBuilder::configureNetworkLayers(const InferenceEngine::InputsDataMap &inputs_info,
    ...
    if (inputs_info.size() == 1) {
        if (input_layer_precision_config.size() > 1)
            throw std::runtime_error("'input_preproc' has size greater than 1. Model has 1 input layer.");

        auto info = inputs_info.begin();
        GVA_INFO("Input image layer name: '%s'", info->first.c_str());
        if (input_layer_precision_config.empty()) {
            info->second->setPrecision(InferenceEngine::Precision::U8);
        } else {
            info->second->setPrecision(getIePrecision(input_layer_precision_config.cbegin()->second));
        }
