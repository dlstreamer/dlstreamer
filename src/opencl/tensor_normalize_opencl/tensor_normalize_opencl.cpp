...
#include "dlstreamer/opencl/elements/tensor_normalize_opencl.h"
...
class TensorNormalizeOpenCL : public BaseTransform {
  public:
    TensorNormalizeOpenCL(DictionaryCPtr /*params*/, const ContextPtr &app_context) : BaseTransform(app_context) {
    }

    FrameInfoVector get_input_info() override {
        if (_output_info.tensors.empty()) {
            return tensor_normalize_opencl.input_info;
        } else {
    ...
    FrameInfoVector get_output_info() override {
        if (_input_info.tensors.empty()) {
            return tensor_normalize_opencl.output_info;
        } else {
...
extern "C" {
DLS_EXPORT ElementDesc tensor_normalize_opencl = {.name = "tensor_normalize_opencl",
                                                  .description =
                                                  ...
                                                  .create = create_element<TensorNormalizeOpenCL>,
