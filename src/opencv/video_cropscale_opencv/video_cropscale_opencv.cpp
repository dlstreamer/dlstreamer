...
#include "dlstreamer/opencv/elements/video_cropscale_opencv.h"
...
class VideoCropScaleOpenCV : public BaseTransform {
  public:
    VideoCropScaleOpenCV(DictionaryCPtr params, const ContextPtr &app_context) : BaseTransform(app_context) {
    ...
    FrameInfoVector get_input_info() override {
        if (_output_info.tensors.empty()) {
            return video_cropscale_opencv.input_info;
        } else {
    ...
    FrameInfoVector get_output_info() override {
        if (_input_info.tensors.empty()) {
            return video_cropscale_opencv.output_info;
        } else {
...
extern "C" {
ElementDesc video_cropscale_opencv = {.name = "video_cropscale_opencv",
                                      .description =
                                          "Fused video crop and scale on OpenCV backend. "
                                      ...
                                      .create = create_element<VideoCropScaleOpenCV>,
