...
class ObjectTracker {
  ...
  public:
     ...
     * @exception std::invalid_argument Input frame size is invalid.
     */
    std::vector<Object> Track(cv::Size frame_size,
                              const std::vector<DetectedObject> &detected_objects = std::vector<DetectedObject>());
     ...
     * 0.5].
     */
    void SetFrameDeltaTime(float frame_delta_t);
     ...
     */
    float GetFrameDeltaTime() const noexcept;
...
class ObjectTracker::Builder {
  ...
  public:
     ...
     */
    std::unique_ptr<ObjectTracker> Build(TrackingType tracking_type) const;
