...
static ParamDescVector params_desc = {
    {param::labels, "Comma-separated list of object classes", std::vector<std::string>()},
    {param::threshold,
...
class PostProcYolo : public BaseTransformInplace {
  ...
  public:
    PostProcYolo(DictionaryCPtr params, const ContextPtr &app_context) : BaseTransformInplace(app_context) {
        _labels = params->get(param::labels, std::vector<std::string>());
        _classes_number = _labels.size();
        _confidence_threshold = params->get<double>(param::threshold, param::default_threshold);
        ...
        _do_cls_softmax = params->get<bool>(param::do_cls_softmax, false);
        _output_sigmoid_activation = params->get<bool>(param::output_sigmoid_activation, false);
        _cells_number_x = _cells_number_y = params->get<int>(param::cells_number, 0);
        if (!_cells_number_x)
            _cells_number_x = params->get<int>(param::cells_number_x, 0);
        if (!_cells_number_y)
            _cells_number_y = params->get<int>(param::cells_number_y, 0);
        _bbox_number_on_cell = params->get<int>(param::bbox_number_on_cell, 0);
    }
    ...
    void set_info(const FrameInfo &info) override {
        ...
        if (_anchors.size() != _bbox_number_on_cell * 2 * _info.tensors.size())
  ...
  protected:
    ...
    OutputDimsLayout _dims_layout = OutputDimsLayout::NO;
    ...
    static std::pair<std::vector<size_t>, size_t> getMinBlobDims(const FrameInfo &outputs_info) {
        auto min_size_dims = outputs_info.tensors.begin()->shape;
        size_t min_blob_size = std::numeric_limits<size_t>::max();

        for (const auto &output_info_pair : outputs_info.tensors) {
    ...
    static OutputDimsLayout getLayoutFromDims(const FrameInfo &outputs_info, const std::vector<double> &anchors,
                                              size_t classes) {
        ...
        size_t boxes = anchors.size() / (outputs_info.tensors.size() * 2);
    ...
    bool tryAutomaticConfig() {
        _bbox_number_on_cell = _anchors.size() / (_info.tensors.size() * 2);
        _cells_number_x = 0;
        _cells_number_y = 0;
