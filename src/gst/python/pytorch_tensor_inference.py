...
def tensor_info_to_gst_caps(tensors_info: List[TensorInfo]) -> Gst.Caps:
    ...
    shapes_str = ",".join(":".join(str(d) for d in tensor_info.shape[::-1]) for tensor_info in tensors_info)
...
class InferencePyTorch(GstBase.BaseTransform):
    ...
    def __init__(self, gproperties = __gproperties__):
        ...
        self.property = { key:value[3] for key, value in gproperties.items() } # default values
    ...
    def init_pytorch_model(self):
        ...
        if is_pytorch_model(model_str):
            ...
            if len(model_name_arr) < 2:
                raise AttributeError(f"Invalid module name in property 'model'. It must include at least the name of the import module and the name of the model. Got: {model_str}")
            ...
            if self.property["model-weights"]:
                ...
                if weights_class:
                    ...
                    Gst.warning("No suitable class with weights was found in the module. No pre-trained weights are used")
    ...
    def get_output_tensors_info(self, input_tensors_info: List[TensorInfo]) -> List[TensorInfo]:
        if len(input_tensors_info) != 1: # TODO: remove limitation. Support models with multiple inputs
            raise RuntimeError(f"Models with one layer only are supported now. Got tensors array from caps size of: {len(input_tensors_info)}")
    ...
    def get_input_tensor_info_from_model(self) -> List[TensorInfo]:
        ...
        if len(resize_size) != 1:
            Gst.warning("Unsupported resize size. Unable to parse preprocessing info from weights. Default values from caps will be used")
        else:
            input_tensors_info.append(TensorInfo([3, resize_size[0], resize_size[0]], torch.float32, []))
    ...
    def do_generate_output(self):
        try:
            ...
            for mem in mems:
                ...
                nd_arr = np.ndarray(shape=input_tensor_info.shape, buffer=map.data, dtype=input_tensor_info.data_type)
                tensor = torch.Tensor(nd_arr, device=self.device).float().unsqueeze(0) # FIXME: support batching
                ...
                if isinstance(output_tensor, dict):
                    ...
                    raise RuntimeError(f"Unsupported inference output type: '{type(output_tensor)}'")
    ...
    def append_tensor_to_buffer(self, buf: Gst.Buffer, tensor: torch.Tensor):
        ...
        np.copyto(np.ndarray(shape=tensor_nd_arr.shape, buffer=map.data, dtype=tensor_nd_arr.dtype), tensor_nd_arr)
        mem.unmap(map)
