...
// GST_DEBUG_CATEGORY_STATIC(splitjoinbin_debug);
//#define GST_CAT_DEFAULT splitjoinbin_debug
...
static void splitjoinbin_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec);
static void splitjoinbin_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);
static GstStateChangeReturn splitjoinbin_change_state(GstElement *element, GstStateChange transition);
static void splitjoinbin_dispose(GObject *object);

static void splitjoinbin_class_init(GstSplitJoinBinClass *klass) {
    ...
    gobject_klass->set_property = splitjoinbin_set_property;
    gobject_klass->get_property = splitjoinbin_get_property;
    gobject_klass->dispose = splitjoinbin_dispose;
    gstelement_klass->change_state = splitjoinbin_change_state;
    ...
    gst_element_class_set_static_metadata(
        gstelement_klass, "Generic split-join bin element", "Generic split-join bin element",
        "Bin element for splitting and merging two pipelines: "
        "tee name=t ! <preprocess> ! <process> ! <postprocess> ! <aggregate>   t. ! aggregate",
        "Intel Corporation");
}

gboolean splitjoinbin_sink_pad_event_function(GstPad *pad, GstObject *parent, GstEvent *event);

static void splitjoinbin_init(GstSplitJoinBin *self) {
    ...
    gst_pad_set_event_function(self->sink_pad, splitjoinbin_sink_pad_event_function);
...
gboolean splitjoinbin_link_elements(GstSplitJoinBin *self) {
    ...
    if (splitjoinbin_is_linked(self))
        return TRUE;
...
static GstStateChangeReturn splitjoinbin_change_state(GstElement *element, GstStateChange transition) {
    GstSplitJoinBin *self = GST_SPLITJOINBIN(element);

    switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:
    case GST_STATE_CHANGE_READY_TO_PAUSED:
        // try link elements
        splitjoinbin_link_elements(self);
        break;
    case GST_STATE_CHANGE_PAUSED_TO_PLAYING:
        // if elements not linked at this stage, it's error
        if (!splitjoinbin_link_elements(self)) {
...
gboolean splitjoinbin_sink_pad_event_function(GstPad *pad, GstObject *parent, GstEvent *event) {
    GstSplitJoinBin *self = GST_SPLITJOINBIN(parent);
...
gboolean splitjoinbin_set_elements(GstSplitJoinBin *self, GstElement *preprocess, GstElement *process,
    ...
    return splitjoinbin_link_elements(self);
}

static GstElement *create_element_from_description(GstSplitJoinBin *self, const gchar *description) {
    GstElement *element;
    GError *error = NULL;
...
static void splitjoinbin_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec) {
    GstSplitJoinBin *self = GST_SPLITJOINBIN(object);
...
G_DEFINE_TYPE(GstSplitJoinBin, splitjoinbin, GST_TYPE_BIN);
