...
std::vector<Object> ObjectTracker::Impl::Track(const cv::Mat &frame,
    ...
    if (frame.cols <= 0 || frame.rows <= 0) {
        printf("Error: Invalid frame size(%dx%d) empty(%d)", frame.cols, frame.rows, frame.empty());
        ETHROW(false, invalid_argument, "Invalid frame size(%dx%d) empty(%d)", frame.cols, frame.rows, frame.empty());
    }
