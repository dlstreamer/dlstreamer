...
gst-launch-1.0 --gst-plugin-path ${GST_PLUGIN_PATH} \
  ...
  gvaclassify model=$CLASS_MODEL_PATH model-proc=$(PROC_PATH $MODEL2) object-class=vehicle ! queue ! \
  gvaclassify model=$CLASS_MODEL_PATH1 model-proc=$(PROC_PATH $MODEL3) object-class=license-plate ! queue ! \
