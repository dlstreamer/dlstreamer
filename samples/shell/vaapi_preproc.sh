...
gst-launch-1.0 --gst-plugin-path ${GST_PLUGIN_PATH} \
                filesrc location=$FILE ! qtdemux ! vaapih264dec ! vaapipostproc ! video/x-raw\(memory:VASurface\) ! \
                gvadetect inference-id=inf0 model=$DETECT_MODEL_PATH model-proc=$(PROC_PATH $MODEL1_PROC) device=$DEVICE pre-proc=$PRE_PROC every-nth-frame=1 batch-size=1 ! queue ! \
                ...
                gvafpscounter ! vaapipostproc ! video/x-raw ! gvawatermark ! videoconvert ! fpsdisplaysink video-sink=xvimagesink sync=false
