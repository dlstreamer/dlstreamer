...
if [ -z ${1} ]; then
  ...
  echo "Usage : ./benchmark.sh VIDEO_FILE [DECODE_DEVICE] [INFERENCE_DEVICE] [CHANNELS_COUNT] [PROCESSES_COUNT] [CODEC_NAME]"
...
if [ $CODEC_NAME == decodebin ]; then
  ...
  if [ $DECODE_DEVICE == CPU ]; then
    DECODER+="$CODEC_NAME ! video/x-raw"
  else
    DECODER+="$CODEC_NAME ! video/x-raw\(memory:DMABuf\)"
  fi
fi
...
if [ $PROCESSES_COUNT -gt 1 ]; then
  if [ $INFERENCE_DEVICE == "CPU" ]; then
    THREADS_NUM=$((($CORES / $PROCESSES_COUNT - $CORES / $PROCESSES_COUNT % $CHANNELS_PER_PROCESS)))
