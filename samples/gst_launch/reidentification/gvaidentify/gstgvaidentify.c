...
#define ELEMENT_DESCRIPTION                                                                                            \
    "Sample element showing how to access inference results generated by gvadetect / gvaclassify. Compares "           \
    "embeddings generated from a reidentification model against a gallery of known objects"
#define ELEMENT_LONG_NAME ELEMENT_DESCRIPTION
...
#define DEFAULT_MIN_COSINE_THRESHOLD -1.
#define DEFAULT_MAX_COSINE_THRESHOLD 1.
#define DEFAULT_COSINE_THRESHOLD 0.3
...
enum { PROP_0, PROP_MODEL, PROP_GALLERY, PROP_COSINE_THRESHOLD };
...
static void gst_gva_identify_class_init(GstGvaIdentifyClass *klass) {
    ...
    gst_element_class_add_pad_template(
        element_class, gst_pad_template_new("src", GST_PAD_SRC, GST_PAD_ALWAYS, gst_caps_from_string("video/x-raw")));
    gst_element_class_add_pad_template(
        element_class, gst_pad_template_new("sink", GST_PAD_SINK, GST_PAD_ALWAYS, gst_caps_from_string("video/x-raw")));
    ...
    g_object_class_install_property(gobject_class, PROP_GALLERY,
                                    g_param_spec_string("gallery", "Gallery",
                                                        "Path to JSON file with list of image examples for each known "
                                                        "object/face/person. See samples for JSON format examples",
                                                        DEFAULT_GALLERY, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(
        gobject_class, PROP_COSINE_THRESHOLD,
        g_param_spec_float("cosine-distance-threshold", "Cosine Distance Threshold",
                           "Threshold for identification results. Only objects with a "
                           "cosine distance less than the threshold will be considered as matches",
                           DEFAULT_MIN_COSINE_THRESHOLD, DEFAULT_MAX_COSINE_THRESHOLD, DEFAULT_COSINE_THRESHOLD,
...
static void gst_gva_identify_reset(GstGvaIdentify *gvaidentify) {
    ...
    gvaidentify->cosine_dist_threshold = DEFAULT_COSINE_THRESHOLD;
    gvaidentify->identifier = NULL;
    gvaidentify->initialized = FALSE;
}
...
static void gst_gva_identify_set_model(GstGvaIdentify *gvaidentify, const gchar *model_path) {
    ...
    if (check_gva_identify_stopped(gvaidentify)) {
        if (model_path != NULL) {
            ...
            GST_INFO_OBJECT(gvaidentify, "Model : %s", gvaidentify->model);
        } else {
...
static void gst_gva_inference_set_gallery(GstGvaIdentify *gvaidentify, const gchar *gallery_path) {
    if (check_gva_identify_stopped(gvaidentify)) {
        if (gallery_path != NULL) {
            ...
            GST_INFO_OBJECT(gvaidentify, "Gallery: %s", gvaidentify->gallery);
        } else {
...
void gst_gva_identify_set_property(GObject *object, guint property_id, const GValue *value, GParamSpec *pspec) {
    ...
    case PROP_GALLERY:
        ...
        break;
    case PROP_COSINE_THRESHOLD:
        gvaidentify->cosine_dist_threshold = g_value_get_float(value);
        break;
    default:
...
void gst_gva_identify_get_property(GObject *object, guint property_id, GValue *value, GParamSpec *pspec) {
    ...
    case PROP_MODEL:
        ...
        break;
    case PROP_COSINE_THRESHOLD:
        g_value_set_float(value, gvaidentify->cosine_dist_threshold);
        break;
    case PROP_GALLERY:
