...
int main(int argc, char *argv[]) {
...
#ifdef __linux__
    const std::string sink_rank = (!(system("pstree -s $$ | grep -q 'sshd'"))) ? "ximagesink:MAX" : "vaapisink:MAX";
    char *user_feature_rank = getenv("GST_PLUGIN_FEATURE_RANK");
    if (user_feature_rank) {
        strcat(user_feature_rank, ",");
        strcat(user_feature_rank, sink_rank.c_str());
        setenv("GST_PLUGIN_FEATURE_RANK", user_feature_rank, 1);
    } else {
        setenv("GST_PLUGIN_FEATURE_RANK", sink_rank.c_str(), 1);
    }
#endif
    // Parse arguments
    ...
    auto launch_str = g_strdup_printf("%s ! %s ! capsfilter caps=\"%s\" ! "
                                      ...
                                      "%s"
                                      "gvawatermark name=gvawatermark ! videoconvert n-threads=4 ! gvafpscounter ! %s",
                                      video_source, preprocess_pipeline, capfilter, detection_model, device, batch_size,
                                      classify_str.c_str(), sink);
    g_print("GST_PLUGIN_FEATURE_RANK=%s PIPELINE: %s \n", getenv("GST_PLUGIN_FEATURE_RANK"), launch_str);
