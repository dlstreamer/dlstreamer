...
class InferenceOpenVINO(GstBase.BaseTransform):
    __gstmetadata__ = ('OpenVINO inference', 'Transform', 'OpenVINO™ toolkit inference element', 'dkl')
    ...
    def do_transform_caps(self, direction, caps, filter):
        ...
        shapes = [":".join(str(d) for d in np.array(info.shape)[::-1]) for info in infos] # dims in reverse order
        types = [self.TYPE_NAME[info.element_type.get_type_name()] for info in infos]
        caps_str = 'other/tensors,num_tensors={num},types={types},dimensions={shapes}'.format(
    ...
    def do_generate_output(self):
        ...
        tensors = [ np.ndarray(shape=info.shape, buffer=map.data, dtype=np.uint8) for map, info in zip(maps, self.model.inputs) ]
