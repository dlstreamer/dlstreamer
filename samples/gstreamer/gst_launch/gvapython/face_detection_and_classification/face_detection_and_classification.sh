...
if [[ $OUTPUT == "display" ]] || [[ -z $OUTPUT ]]; then
  ...
  FILE="$(basename ${INPUT%.*})"
  rm -f "gvapython_${FILE}_${DEVICE}.mp4"
  ...
  SINK_ELEMENT="gvawatermark ! videoconvertscale ! gvafpscounter ! ${ENCODER} ! avimux name=mux ! filesink location=gvapython_${FILE}_${DEVICE}.mp4"
else
