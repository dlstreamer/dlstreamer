...
constexpr char plugin_name[] = "gvaclassify";
...
GST_START_TEST(test_threshold_property_lower_min) {
    g_print("Starting test: test_threshold_property_lower_min\n");
    GValue prop_value = G_VALUE_INIT;
    g_value_init(&prop_value, G_TYPE_FLOAT);
...
static Suite *element_invalid_properties_testing_suite(void) {
    Suite *s = suite_create("element_invalid_properties_testing");
    ...
    tcase_add_test(tc_chain, test_threshold_property_lower_min);

    return s;
}

GST_CHECK_MAIN(element_invalid_properties_testing);
