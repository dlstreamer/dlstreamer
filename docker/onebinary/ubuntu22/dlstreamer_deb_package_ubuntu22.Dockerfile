...
LABEL description="This is the one binary image of Intel® Deep Learning Streamer (Intel® DL Streamer) Pipeline Framework"
...
#Build GStreamer
WORKDIR ${GSTREAMER_DIR}/src

RUN \
    git clone https://gitlab.freedesktop.org/gstreamer/gstreamer.git gstreamer-${GST_VERSION}

WORKDIR ${GSTREAMER_DIR}/src/gstreamer-${GST_VERSION}
...
ENV LIBDIR=${DLSTREAMER_DIR}/build/intel64/Release/lib
ENV BINDIR=${DLSTREAMER_DIR}/build/intel64/Release/bin
...
ENV LD_LIBRARY_PATH=${GSTREAMER_DIR}/lib:${LIBDIR}:/usr/lib:/opencv/build/3rdparty/lib:${LD_LIBRARY_PATH}
ENV LIB_PATH=$LIBDIR
...
RUN \
    ...
    chown -R dlstreamer ${DLSTREAMER_DIR}
