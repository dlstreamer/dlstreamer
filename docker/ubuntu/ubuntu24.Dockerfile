...
RUN \
    apt-get update && \
    apt-get install -y -q --no-install-recommends ffmpeg=\* librdkafka-dev=\* libpaho-mqtt-dev=\* libopencv-dev=\* && \
    apt-get clean && \
    rm -rf /var/lib/apt/lists/*
...
RUN \
    apt-get update && \
    apt-get install -y -q --no-install-recommends curl=\* gcc=\* cmake=\* libtbb12=\* gpg=\* ca-certificates=\* && \
    apt-get clean && \
    rm -rf /var/lib/apt/lists/*
...
ENV LD_LIBRARY_PATH=/opt/intel/dlstreamer/gstreamer/lib:/opt/intel/dlstreamer/build/intel64/Release/lib:/opt/intel/dlstreamer/lib/gstreamer-1.0:/usr/lib:/opt/intel/dlstreamer/build/intel64/Release/lib:/usr/local/lib/gstreamer-1.0:/usr/local/lib
ENV LIBVA_DRIVERS_PATH=/usr/lib/x86_64-linux-gnu/dri
ENV GST_VA_ALL_DRIVERS=1
